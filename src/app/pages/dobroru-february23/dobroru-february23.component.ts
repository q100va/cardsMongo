...
@Component({
  selector: "app-dobroru-february23",
  templateUrl: "./dobroru-february23.component.html",
  styleUrls: ["./dobroru-february23.component.css"],
})
export class DobroruFebruary23Component implements OnInit {
  order: Order;
  userName: string;
  form: FormGroup;
  holiday: string = "23 февраля 2026";
  lineItems = [];
  types: Array<string> = [
    "email",
    "phoneNumber",
  ...
  getAddresses() {
    ...
    if (!this.isForInstitutes) {
      ...
      bottom =
        "Отправляйте письма правильно!\n – Открытки отправляйте Почтой России только ПРОСТЫМИ письмами/открытками (НЕ заказными).\n – Обращаем ваше внимание, что c 10 марта 2026 г. 'Почта России' поднимает цены на пересылку.\n  – Каждому адресату по возможности отправляйте отдельную открытку в отдельном конверте или отдельную почтовую открытку без конверта.\n – Если у вас много адресатов из одного интерната, можно отправить в один интернат все открытки одной ПРОСТОЙ бандеролью.\n – Учтите, что по России письма идут 30 дней и дольше. Рекомендуем отправить открытки в ближайшие дни с пометкой на конверте 'прошу вручить 31 декабря'.\n\n" +
