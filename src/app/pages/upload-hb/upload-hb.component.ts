...
export class UploadHbComponent implements OnInit {
  ...
  userName: string;
  form: FormGroup;
  holiday: string = "Дни рождения ноября 2024";
  lineItems = [];
  types: Array<string> = [
    "email",
    /*     "phoneNumber",
  ...
  goNext(event) {
    ...
    console.log("click");
    if (this.isMainMonth) {
      this.holiday = "Дни рождения ноября 2024";
    }
    if (this.isNextMonth) {
  ...
  goBack(event) {
    ...
    if (this.isBeforeMonth) {
      this.holiday = "Дни рождения октября 2024";
    }
  }
  ...
  sendEmail() {
    ...
    const subscribersTopText =
      "К сожалению, сроки доставки писем в настоящее время предсказать невозможно.Некоторые интернаты сообщают, что корреспонденция опаздывает на 2-3 недели,а где-то больше, чем на месяц. Хотя какие-то письма доходят довольно быстро. Поэтому просим вас отправлять открытки минимум за месяц до праздника или дня рождения.На конверте рекомендуем делать пометку 'прошу вручить <дата>'.\n\n" + this.holiday + "\n\n";
    ...
    "<div>Спасибо вам преогромное!<br>___________________________________________________________<br>Помочь оплатить труд культорга для отделения милосердия можно по ссылке:<br><a href = 'https://starikam.org/campaign/poka-burlit-zhizn-toske-net-mesta/'>Пока бурлит жизнь, тоске нет места</a></div><br>";
    ...
    const subscribersInstituteBottomHtml =
      ...
      "<div>_______________________________________________________________________________________<br>Помочь оплатить труд культорга для отделения милосердия можно по ссылке:<br><a href = 'https://starikam.org/campaign/poka-burlit-zhizn-toske-net-mesta/'>Пока бурлит жизнь, тоске нет места</a> </div><br>";
  ...
  getAddresses() {
    ...
    if (!this.isForInstitutes) {
      ...
      bottom =
        ...
        "Нашим подопечным нужны продукты, дрова для обогрева домов, медицинская помощь. Сделайте пожертвование на - https://starikam.org/ \n\n" +
