...
export class UploadHbComponent implements OnInit {
  ...
  sendEmail() {
    ...
    const subscribersTopText =
      "Сроки доставки писем в настоящее время, к сожалению, предсказать невозможно. Некоторые интернаты сообщают, что корреспонденция опаздывает на 2-3 недели,а где-то больше, чем на месяц. Поэтому просим вас отправлять открытки минимум за месяц до праздника или дня рождения.На конверте рекомендуем делать пометку 'прошу вручить <дата>'.\n\n" + this.holiday + "\n\n";
    ...
    "<div>Сердечно вас благодарю!<br>___________________________________________________________<br>Помочь оплатить труд культорга для отделения милосердия можно по ссылке:<br><a href = 'https://starikam.org/campaign/poka-burlit-zhizn-toske-net-mesta/'>Пока бурлит жизнь, тоске нет места</a></div><br>";
