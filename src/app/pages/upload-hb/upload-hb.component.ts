...
export class UploadHbComponent implements OnInit {
  ...
  sendEmail() {
    ...
    const subscribersTopText =
      "К сожалению, сроки доставки писем в настоящее время предсказать невозможно. Некоторые интернаты сообщают, что корреспонденция опаздывает на 2-3 недели,а где-то больше, чем на месяц. Поэтому просим вас отправлять открытки минимум за месяц до праздника или дня рождения.На конверте рекомендуем делать пометку 'прошу вручить <дата>'.\n\n" + this.holiday + "\n\n";
