...
export class FamilyDayComponent implements OnInit {
  ...
  userName: string;
  form: FormGroup;
  holiday: string = "День семьи 2024";
  ...
  ngOnInit(): void {
    ...
    this.listService.findAllFamilyDayLists().subscribe(
      (res) => {
        this.familyDay = res["data"];
        this.familyDay.sort((prev, next) => prev.nursingHome - next.nursingHome);
        this.dataSource = new MatTableDataSource(this.familyDay);
      },
      (err) => {
        console.log(err);
      }
    ...
    for (let lineItem of this.lineItems) {
      ...
      for (let celebrator of lineItem.celebrators) {
        addresses =
          ...
          celebrator.husbandLastName +
          " " +
          celebrator.husbandFirstName +
          " " +
          celebrator.husbandPatronymic +
          " (" +
          celebrator.husbandYearBirthday +
          ") и " +
          celebrator.wifeLastName +
          " " +
          celebrator.wifeFirstName +
          " " +
          celebrator.wifePatronymic +
          " (" +
          celebrator.wifeYearBirthday +
          ") " +
          celebrator.comment1 +
