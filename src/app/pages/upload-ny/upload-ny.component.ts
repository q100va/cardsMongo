...
export class UploadNyComponent implements OnInit {
  ...
  sendEmail() {
    ...
    const subscribersTopHtml =
      "<div>К сожалению, сроки доставки писем в настоящее время предсказать невозможно.<br>Некоторые интернаты сообщают, что корреспонденция опаздывает на 2-3 недели, а где-то больше, чем на месяц.<br>Поэтому прошу вас отправить открытки в ноябре - начале декабря и только простыми отправлениями.<br>На конверте можно указать 'вручить 31 декабря'.</div><br><p style='font-weight: bold;'>" +
      this.holiday +
      "</p>";
