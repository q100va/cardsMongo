...
export class UploadNyComponent implements OnInit {
  ...
  sendEmail() {
    ...
    const subscribersTopText =
      "Прошу вас отправить открытки в начале декабря и только простыми отправлениями.\n\n" +
      this.holiday +
      "\n\n";
    ...
    const subscribersTopHtml =
      "<div>Прошу вас отправить открытки в начале декабря и только простыми отправлениями.</div><br><p style='font-weight: bold;'>" +
      this.holiday +
      "</p>";
    const subscribersBottomHtml =
      "<div>Спасибо вам преогромное!<br>___________________________________________________________<br>Вы можете купить нужные товары для фонда через Ozon<br>по ссылке <a href = 'https://www.ozon.ru/highlight/pokupayu-pomogayu-1804052/?miniapp=supermarket'>Покупаю — помогаю — интернет-магазин OZON</a></div><br>";
