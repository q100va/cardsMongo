  ...
  getAddresses() {
    ...
    if (!this.isForInstitutes) {
      ...
      topForSubscribers =
        ...
        "Открытки просим вас отправить минимум за месяц до праздника.\n" +
      ...
      topForSubscribers =
        ...
        "Если вам удобнее, то в один интернат можно отправить все открытки одной ПРОСТОЙ бандеролью. Получателем указать интернат. На конверте рекомендуем указать название праздника. Например, 'Пасха'.\n" +
        "Часто работники почты уговаривают отправителей на регистрируемые отправления (заказные, ценные, первого класса), но нам такой вариант совершенно не подходит, так как такие отправления с 99% вероятностью вернутся обратно.\n" +
        "Мы не можем сказать, кто из наших подопечных верующий и какого вероисповедания придерживается. Мы поздравляем всех. Если вы сомневаетесь, что человек празднует Пасху, можно начать ваше поздравление следующим образом: 'Уважаемая ...! Поздравляю Вас с наступившей весной и праздником Пасхи, если Вы отмечаете! Желаю....'\n\n";
    }
