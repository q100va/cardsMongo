...
export class NewYearComponent implements OnInit {
  ...
  getAddresses() {
    ...
    let topForSubscribers =
      ...
      "Поэтому просим вас отправить открытки в конце октября - начале ноября.\n" +
      "А на конверте сделать пометку 'прошу вручить 31 декабря'.\n\n";
    ...
    /*     let topForSubscribers =
      ...
      "Открытки нужно отправить в ближайшие дни.\n\n"; */
    ...
    if (!this.isForInstitutes || this.isParcelAvailable) {
      ...
      /*       topForSubscribers =
        topForSubscribers +
        "На конверте рекомендуем указать название праздника. Например, 'Новый год'.\n\n" +
    ...
    if (!this.isForInstitutes || this.isParcelAvailable) {
      ...
      if (!this.isParcelAvailable) {
        bottom =
          ...
          "Помогите обеспечить горячей едой пожилых людей  https://starikam.org/campaign/lyudyam-nuzhna-goryachaya-eda/\n\n" +
    ...
    let bottomForSubscribers =
      "Многократно благодарим вас за доброе сердце и отзывчивость!!!!\n" +
