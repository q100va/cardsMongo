...
export class NewYearComponent implements OnInit {
  ...
  getAddresses() {
    ...
    let topForSubscribers =
      ...
      "Поэтому просим вас отправить открытки в ближайшие дни.\n\n";

    /*   \n" +
      "А на конверте сделать пометку 'прошу вручить 31 декабря'.  let topForSubscribers =
      "Сроки доставки писем в настоящее время, к сожалению, предсказать невозможно.\n" +
     ...
     /*  } else {
      ...
      if (!this.isParcelAvailable) {
        bottom =
          ...
          "Помогите купить специальные кресла и ванны-тележки  https://starikam.org/campaign/bezopasnyj-dush-i-vanna-s-penoj/\n\n" +
