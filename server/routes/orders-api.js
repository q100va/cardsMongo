...
async function deletePluses(deletedOrder, full) {
  ...
  if (deletedOrder.holiday == "Дни рождения января 2026" || deletedOrder.holiday == "Дни рождения февраля 2026" || deletedOrder.holiday == "Дни рождения декабря 2025") {
    ...
    if (deletedOrder.holiday == "Дни рождения декабря 2025") {
      monthNumber = monthNumber - 1;
    }
...
async function restorePluses(updatedOrder) {
  if (updatedOrder.holiday == "Дни рождения января 2026" || updatedOrder.holiday == "Дни рождения февраля 2026" || updatedOrder.holiday == "Дни рождения декабря 2025") {
    ...
    for (let lineItem of updatedOrder.lineItems) {
      for (let person of lineItem.celebrators) {
        if (person.monthBirthday == month.number) {
          ...
          if (updatedOrder.holiday == "Дни рождения декабря 2025") {
...
async function createOrder(newOrder, prohibitedId, restrictedHouses) {
  ...
  if (newOrder.holiday == "Дни рождения декабря 2025") {
    period = {
      "date1": 26,
      "date2": 31,
      "isActive": true,
...
router.get("/restore-pluses/:holiday", checkAuth, async (req, res) => {
  try {
    ...
    if (req.params.holiday == "birthdayBefore") {
      ...
      for (let celebrator of celebratorsHB) {
        let plusAmount = await Order.find({ "lineItems.celebrators._id": celebrator._id, isDisabled: false, isOverdue: false, isReturned: false, holiday: "Дни рождения декабря 2025" }).countDocuments();
