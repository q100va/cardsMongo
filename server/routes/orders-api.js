          ...
          $or: [
            ...
            },
            {
              ...
              holiday: {
                $in: [
                  //"Новый год 2026",
                  "Дни рождения июня 2026",
                  "Дни рождения июля 2026",
                  "Дни рождения августа 2026",
                  "23 февраля 2026",
                  "8 марта 2026",
                  "9 мая 2026", "Пасха 2026",
                  // 
            ...
            },
            {
              ...
              holiday: {
                $in: [
                  ...
                  "Дни рождения августа 2026",
                  "23 февраля 2026",
                  "8 марта 2026", "9 мая 2026", "Пасха 2026",
                  //
                  /*         "Именины февраля 2025",
                          "Именины марта 2025" */
                ]
...
async function deletePluses(deletedOrder, full) {
  ...
  if (deletedOrder.holiday == "Дни рождения июля 2026" || deletedOrder.holiday == "Дни рождения августа 2026" || deletedOrder.holiday == "Дни рождения июня 2026") {
    ...
    if (deletedOrder.holiday == "Дни рождения августа 2026") {
      monthNumber = monthNumber + 1;
    }
    if (deletedOrder.holiday == "Дни рождения июня 2026") {
      monthNumber = monthNumber - 1;
    }
    for (let lineItem of deletedLineItems) {
      for (let person of lineItem.celebrators) {
        if (person.monthBirthday == monthNumber) {
          ...
          if (deletedOrder.holiday == "Дни рождения июля 2026") {
        ...
        if (deletedOrder.holiday == "Именины января 2025") {
          ...
          if (deletedOrder.holiday == "День учителя и дошкольного работника 2024") {
            ...
            if (deletedOrder.holiday == "Новый год 2026") {
              ...
              if (deletedOrder.holiday == "8 марта 2026") {
                for (let lineItem of deletedLineItems) {
                  for (let person of lineItem.celebrators) {
                ...
                if (deletedOrder.holiday == "23 февраля 2026") {
                  for (let lineItem of deletedLineItems) {
                    for (let person of lineItem.celebrators) {
                  ...
                  if (deletedOrder.holiday == "9 мая 2026") {
                    for (let lineItem of deletedLineItems) {
                      for (let person of lineItem.celebrators) {

                        let senior = await May9.findOne({ _id: person._id });
                        let p = senior.plusAmount;
                        let newP = p - 1;
                    ...
                    if (deletedOrder.holiday == "День семьи 2024") {
                      ...
                      if (deletedOrder.holiday == "Пасха 2026") {
                        for (let lineItem of deletedLineItems) {
                          for (let person of lineItem.celebrators) {
...
async function restorePluses(updatedOrder) {
  if (updatedOrder.holiday == "Дни рождения июля 2026" || updatedOrder.holiday == "Дни рождения августа 2026" || updatedOrder.holiday == "Дни рождения июня 2026") {
    ...
    const inTwoWeeks = new Date();
    let period, activePeriod, celebrator;
    if (updatedOrder.holiday == "Дни рождения августа 2026") {
      monthNumber = monthNumber + 1;
    }
    ...
    for (let lineItem of updatedOrder.lineItems) {
      for (let person of lineItem.celebrators) {
        if (person.monthBirthday == month.number) {
          ...
          if (updatedOrder.holiday == "Дни рождения июля 2026") {
            await List.updateOne({ _id: person.celebrator_id }, { $inc: { plusAmount: +1 } }, { upsert: false });
          }
          if (updatedOrder.holiday == "Дни рождения июня 2026") {
        ...
        if (updatedOrder.holiday == "Именины марта 2025") {
          ...
          if (updatedOrder.holiday == "День учителя и дошкольного работника 2024") {
            ...
            if (updatedOrder.holiday == "Новый год 2026") {
              ...
              if (updatedOrder.holiday == "8 марта 2026") {
                for (let lineItem of updatedOrder.lineItems) {
                  for (let person of lineItem.celebrators) {
                    await March8.updateOne({ _id: person._id }, { $inc: { plusAmount: +1 } }, { upsert: false });
                ...
                if (updatedOrder.holiday == "23 февраля 2026") {
                  for (let lineItem of updatedOrder.lineItems) {
                    for (let person of lineItem.celebrators) {
                      await February23.updateOne({ _id: person._id }, { $inc: { plusAmount: +1 } }, { upsert: false });
                  ...
                  if (updatedOrder.holiday == "9 мая 2026") {
                    for (let lineItem of updatedOrder.lineItems) {
                      for (let person of lineItem.celebrators) {
                        await May9.updateOne({ _id: person._id }, { $inc: { plusAmount: +1 } }, { upsert: false });
                      }
                    ...
                    if (updatedOrder.holiday == "День семьи 2024") {
                      ...
                      if (updatedOrder.holiday == "Пасха 2026") {
                        for (let lineItem of updatedOrder.lineItems) {
                          for (let person of lineItem.celebrators) {
                            console.log("person");
                            console.log(person);
  ...
  try {
    ...
    if (order) {
      if (order.temporaryLineItems && order.temporaryLineItems.length > 0) {
        ...
        if (holiday == "Дни рождения августа 2026") {
...
async function createOrder(newOrder, prohibitedId, restrictedHouses) {
  ...
  if (newOrder.holiday == "Дни рождения июня 2026") {
    period = {
      "date1": 26,
      "date2": 30,
      "isActive": true,
  ...
  if (newOrder.holiday == "Дни рождения августа 2026") {
    period = {
      "date1": 1,
      "date2": 5,
      "isActive": true,
...
async function fillOrder(housesForInstitutes, proportion, period, order_id, filter, prohibitedId, restrictedHouses, orderFilter, holiday) {
  ...
  for (let category of categories) {
    ...
    if (proportion[category]) {
      ...
      data = await collectSeniors(housesForInstitutes, data, orderFilter, holiday);
      console.log("data.counter - не хватает");
      console.log(data.counter);

      if (data.counter < proportion[category]) {
        // data.maxPlus = data.maxPlus + 1;
        // console.log("data.maxPlus - не хватает");
        // console.log(data.maxPlus);
...
router.post("/spring/:amount", checkAuth, async (req, res) => {
  ...
  try {
    ...
    if (!finalResult) {
      ...
      if (req.body.holiday == "Пасха 2026") { answer = await deleteErrorPlusEaster(false, req.body.userName) };
      if (req.body.holiday == "9 мая 2026") { answer = await deleteErrorPlusVeterans(false, req.body.userName) };
      console.log("answer");
      console.log(answer);
      if (!answer) {
        text = 'Произошла ошибка, но, скорее всего заявка была сформирована и сохранена. Проверьте страницу "Мои заявки" и сообщите об ошибке администратору.'
  ...
  try {
    ...
    if (order) {
      if (order.temporaryLineItems && order.temporaryLineItems.length > 0) {
        ...
        if (order.holiday == "23 февраля 2026") {
...
async function createOrderSpring(newOrder, prohibitedId, restrictedHouses) {
  ...
  if (seniorsData.celebratorsAmount < newOrder.amount) {
    ...
    if (newOrder.holiday == "23 февраля 2026" || newOrder.holiday == "8 марта 2026") { await deleteErrorPlusSpring(order_id); };
    if (newOrder.holiday == "Пасха 2026") { await deleteErrorPlusEaster(order_id) };
    if (newOrder.holiday == "9 мая 2026") { await deleteErrorPlusVeterans(order_id) };
  ...
  for (let kind of searchOrders[data.category]) {
    ...
    outer1: for (let i = 0; i < barrier; i++) {
      ...
      if (result) {
        ...
        if (data.holiday == "23 февраля 2026") {
...
router.get("/restore-pluses/:holiday", checkAuth, async (req, res) => {
  try {
    ...
    if (req.params.holiday == "birthday") {
      const celebratorsHB = await List.find({ absent: false, nursingHome: "КРАСНАЯ_ДУБРАВА_ДЕТИ" });// 
      for (let celebrator of celebratorsHB) {
        let plusAmount = await Order.find({ "lineItems.celebrators._id": celebrator._id, isDisabled: false, isOverdue: false, isReturned: false, holiday: "Дни рождения июля 2026" }).countDocuments();
    ...
    if (req.params.holiday == "birthdayBefore") {
      ...
      for (let celebrator of celebratorsHB) {
        let plusAmount = await Order.find({ "lineItems.celebrators._id": celebrator._id, isDisabled: false, isOverdue: false, isReturned: false, holiday: "Дни рождения июня 2026" }).countDocuments();
    ...
    if (req.params.holiday == "birthdayBefore") {
      ...
      for (let celebrator of celebratorsHB) {
        let plusAmount = await Order.find({ "lineItems.celebrators._id": celebrator._id, isDisabled: false, isOverdue: false, isReturned: false, holiday: "Дни рождения августа 2026" }).countDocuments();
    ...
    if (req.params.holiday == "easter") {
      ...
      for (let celebrator of celebratorsEaster) {
        ...
        let plusAmount = await Order.find({ "lineItems.celebrators.seniorId": celebrator.seniorId, isDisabled: false, isOverdue: false, isReturned: false, holiday: "Пасха 2026" }).countDocuments();
    ...
    if (req.params.holiday == "veterans") {
      ...
      for (let celebrator of celebratorsMay9) {
        ...
        /*         console.log(celebrator.seniorId);
                let plusAmount = await Order.find({ "lineItems.celebrators.seniorId": celebrator.seniorId, isDisabled: false, isOverdue: false, isReturned: false, holiday: "9 мая 2026" }).countDocuments();
...
async function fillOrderForInstitutes(
  ...
  for (let house of activeHouse) {
    ...
    if (filter.onlyWithConcent) {
      if (holiday == "23 февраля 2026" && !filter.region && filter.minNumberOfHouses) { //&& filter.addressFilter == "noSpecial"
    ...
    if (filter.onlyWithConcent) {
      if (holiday == "8 марта 2026" && !filter.region && filter.minNumberOfHouses) { //&& filter.addressFilter == "noSpecial"
    ...
    if (filter.onlyWithConcent) {
      if (holiday == "9 мая 2026" && !filter.region && filter.minNumberOfHouses) { //&& filter.addressFilter == "noSpecial"
    ...
    if (filter.onlyWithConcent) {
      if (holiday == "Пасха 2026" && !filter.region && filter.minNumberOfHouses) { //&& filter.addressFilter == "noSpecial"
...
/*       if (contact == "@tterros") {
  ...
  if ((holiday == "23 февраля 2026" || holiday == "8 марта 2026" || holiday == "Пасха 2026" || holiday == "9 мая 2026") && (filter.noNames || filter.minNumberOfHouses)) {
    ...
    for (let senior of seniorsData) {
      ...
      if (holiday != "9 мая 2026") {
        ...
        let p = newP - 1;
        let c = senior.category;
        let h = (holiday == "Пасха 2026") ? "easter" : "spring";
