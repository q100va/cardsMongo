...
async function findAllMonthCelebrators(month) {
  ...
  let notActiveHouses = await House.find({ $or: [{ isActive: false }, { dateLastUpdate: { $lt: new Date("2025-9-1") } }] });
  ...
  for (let celebrator of list) {
    ...
    let cloneCelebrator = {
      ...
      holyday: month == 5 ? 'Дни рождения мая 2026' : 'Дни рождения июня 2026',
  ...
  let finalList;
  if (month == 4) { finalList = await ListBefore.insertMany(newList, options); }
  if (month == 5) { finalList = await List.insertMany(newList, options); }
  if (month == 6) { finalList = await ListNext.insertMany(newList, options); }
...
async function specialComment(age) {
  ...
  //if(age == 65) console.log('AGE 65', age % 5);
  if (age % 5 === 0) {
...
async function findAllMonthNameDays(month) {
  ...
  for (let celebrator of list) {
    ...
    let holiday;
    if (month == 5) holiday = 'Именины марта 2025';
    if (month == 6) holiday = 'Именины апреля 2025';
  ...
  const options = { ordered: false };
  let finalList;
  if (month == 5) finalList = await NameDay.insertMany(newList, options);
  if (month == 6) finalList = await NameDayNext.insertMany(newList, options);
...
async function createCloneCelebrator(celebrator) {
  ...
  let holiday;
  if (celebrator.monthBirthday == 4) { holiday = 'Дни рождения апреля 2026' };
...
async function checkAllHBFullness(house) {
  ...
  /*   Senior.updateMany({ nursingHome: nursingHome.nursingHome }, { $set: { noAddress: nursingHome.noAddress, isReleased: nursingHome.isReleased } });
    Model.updateMany({ nursingHome: nursingHome.nursingHome }, { $set: { noAddress: nursingHome.noAddress, isReleased: nursingHome.isReleased } });
    NewYear.updateMany({ nursingHome: nursingHome.nursingHome }, { $set: { noAddress: nursingHome.noAddress, isReleased: nursingHome.isReleased } });
   */
...
router.get("/holiday/special-list", checkAuth, async (req, res) => {
  try {
    ...
    let ordersM = await Order.find({ contact: { $in: ["l.filchukova@starikam.org"] }, isDisabled: false, holiday: ["Дни рождения июня 2026"] });
    let lineItemsM = [];
      ...
      [
        {
          $match:
            ...
            { holiday: "Пасха 2026", isDisabled: false, isOverdue: false, isReturned: false, source: source, }          //   contactType: "telegram"
...
async function countHB() {
  ...
  let plusesAmount = await Order.aggregate(
    [
      {
        $match:
          { holiday: "Дни рождения апреля 2026", isDisabled: false, isOverdue: false, isReturned: false }
      },
      {
        $group: { _id: null, sum_val: { $sum: "$amount" } }
      }
  ...
  let orders = await Order.find({ holiday: "Дни рождения апреля 2026", isDisabled: false, isOverdue: false, isReturned: false });
...
async function countEaster() {
  ...
  let plusesAmount = await Order.aggregate(
    [
      {
        $match: { holiday: "Пасха 2026", isDisabled: false, isOverdue: false, isReturned: false }
      },
      {
        $group: { _id: null, sum_val: { $sum: "$amount" } }
      }
...
async function countVolonteers() {
  ...
  let ordersBirthday = await Order.find({ holiday: "Дни рождения апреля 2026", isDisabled: false, isOverdue: false, isReturned: false, });
  ...
  /*   let ordersInstitutes = await Order.find({ holiday: { $in: ["Дни рождения апреля 2026"] }, institutes: { $ne: [] }, isDisabled: false, isOverdue: false, isReturned: false, });//, "Пасха 2026", "9 мая 2026"
    let ordersSchools = await Order.find({ holiday: { $in: ["Дни рождения апреля 2026"] }, "institutes.category": "образовательное учреждение", isDisabled: false, isOverdue: false, isReturned: false, });   //.project({ _id: 0, email: 1, contact: 1,  }); , "institutes.category": "образовательное учреждение", institutes: { $ne: [] }, dateOfOrder: { $gt: new Date('2023-12-31'), $lt: new Date('2024-02-01') }, "Пасха 2026", "9 мая 2026"
  
   */

  let ordersInstitutes = await Order.find({ holiday: { $in: ["8 марта 2026", "Дни рождения апреля 2026"] }, institutes: { $ne: [] }, isDisabled: false, isOverdue: false, isReturned: false, });//, "Пасха 2026", "9 мая 2026"
  let ordersSchools = await Order.find({ holiday: { $in: ["8 марта 2026", "Дни рождения апреля 2026"] }, "institutes.category": "образовательное учреждение", isDisabled: false, isOverdue: false, isReturned: false, });   //.project({ _id: 0, email: 1, contact: 1,  }); , "institutes.category": "образовательное учреждение", institutes: { $ne: [] }, dateOfOrder: { $gt: new Date('2023-12-31'), $lt: new Date('2024-02-01') }, "Пасха 2026", "9 мая 2026"
...
async function findUncertain() {
  ...
  let list = [];
  let listOfUncertain = [];
  let orders = await Order.find({ holiday: "Дни рождения апреля 2026", isDisabled: false, isAccepted: false, isReturned: false, isOverdue: false });   //.project({ _id: 0, email: 1, contact: 1,  });
  for (let order of orders) {
    for (let lineItem of order.lineItems) {
      for (let celebrator of lineItem.celebrators) {
...
router.get("/statistic", checkAuth, async (req, res) => {
  try {
    ...
    let statistic = [
      {
        ...
        amount1: 0, //Дни рождения апреля 2026
        amount2: 0, //8 марта 2026
        ////   amount8: 0, // 23 февраля 2026
        amount3: 0, //Дни рождения мая 2026
        amount4: 0, //Дни рождения июня 2026
      ...
      },
      {
        ...
        amount1: '-', //Дни рождения апреля 2026
        // amount2: '-', //8 марта 2026        
        amount3: '-', //Дни рождения мая 2026
        amount4: '-', //Дни рождения июня 2026
        amount5: '-', //Пасха 2026
        amount6: 0, //9 мая 2026
    ...
    for (let i = 0; i < holidays.length; i++) {
      if (i != 6 && i != 7) { //1 - March8, 4 - Easter, 5 - May9 &&  i != 5
