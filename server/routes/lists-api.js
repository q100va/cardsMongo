...
async function findAllMonthCelebrators(month) {
  ...
  for (let celebrator of list) {
    ...
    let cloneCelebrator = {
      ...
      holyday: month == 7 ? 'Дни рождения августа 2025' : 'Дни рождения сентября 2025',
...
async function createCloneCelebrator(celebrator) {
  ...
  let holiday;
  if (celebrator.monthBirthday == 7) { holiday = 'Дни рождения июля 2025' };
  if (celebrator.monthBirthday == 8) { holiday = 'Дни рождения августа 2025' };
...
async function makeDoubtAvailable() {
  let doubt = await Order.find({ isDisabled: false, isOverdue: false, isReturned: false, isAccepted: false, holiday: "Новый год 2025" });
...
router.get("/holiday/special-list", checkAuth, async (req, res) => {
  try {
    ...
    let ordersM = await Order.find({ contact: { $in: ["l.filchukova@starikam.org"] }, isDisabled: false, holiday: ["Дни рождения сентября 2025"] });
    let lineItemsM = [];
...
async function reportSources() {
  ...
  let months = ["января", "февраля", "марта", "апреля", "мая", "июня", "июля", "октября"] //, "декабря", "декабря", "декабря", "декабря",
...
async function countHB() {
  ...
  let plusesAmount = await Order.aggregate(
    [
      {
        $match:
          { holiday: "Дни рождения июля 2025", isDisabled: false, isOverdue: false, isReturned: false }
      },
      {
        $group: { _id: null, sum_val: { $sum: "$amount" } }
      }
  ...
  let orders = await Order.find({ holiday: "Дни рождения июля 2025", isDisabled: false, isOverdue: false, isReturned: false });
...
async function countNY() {
  ...
  let plusesAmount = await Order.aggregate(
    [
      {
        $match: { holiday: "Новый год 2025", isDisabled: false, isOverdue: false, isReturned: false }
      },
      {
        $group: { _id: null, sum_val: { $sum: "$amount" } }
      }
...
async function countVolonteers() {
  ...
  let ordersBirthday = await Order.find({ holiday: "Дни рождения июля 2025", isDisabled: false, isOverdue: false, isReturned: false, });
  ...
  /*   let ordersInstitutes = await Order.find({ holiday: { $in: ["Дни рождения июля 2025"] }, institutes: { $ne: [] }, isDisabled: false, isOverdue: false, isReturned: false, });//, "Пасха 2025", "9 мая 2025"
    let ordersSchools = await Order.find({ holiday: { $in: ["Дни рождения июля 2025"] }, "institutes.category": "образовательное учреждение", isDisabled: false, isOverdue: false, isReturned: false, });   //.project({ _id: 0, email: 1, contact: 1,  }); , "institutes.category": "образовательное учреждение", institutes: { $ne: [] }, dateOfOrder: { $gt: new Date('2023-12-31'), $lt: new Date('2024-02-01') }, "Пасха 2025", "9 мая 2025"
  
   */

  let ordersInstitutes = await Order.find({ holiday: { $in: ["8 марта 2025", "Дни рождения июля 2025"] }, institutes: { $ne: [] }, isDisabled: false, isOverdue: false, isReturned: false, });//, "Пасха 2025", "9 мая 2025"
  let ordersSchools = await Order.find({ holiday: { $in: ["8 марта 2025", "Дни рождения июля 2025"] }, "institutes.category": "образовательное учреждение", isDisabled: false, isOverdue: false, isReturned: false, });   //.project({ _id: 0, email: 1, contact: 1,  }); , "institutes.category": "образовательное учреждение", institutes: { $ne: [] }, dateOfOrder: { $gt: new Date('2023-12-31'), $lt: new Date('2024-02-01') }, "Пасха 2025", "9 мая 2025"
...
async function findUncertain() {
  ...
  let list = [];
  let listOfUncertain = [];
  let orders = await Order.find({ holiday: "Дни рождения июля 2025", isDisabled: false, isAccepted: false, isReturned: false, isOverdue: false });   //.project({ _id: 0, email: 1, contact: 1,  });
  for (let order of orders) {
    for (let lineItem of order.lineItems) {
      for (let celebrator of lineItem.celebrators) {
    ...
    let statistic = [
      {
        ...
        amount1: 0, //ДР июля 2025
        // amount2: 0, //8 марта 2025
        amount3: 0, //ДР августа 2025
        amount4: 0, //ДР сентября 2025
        //  amount5: 0, //Пасха 2025
        //  amount6: 0, //9 мая 2025
      },
      {
        name: "в т.ч. ветеранов и детей войны",
        amount1: '-', //ДР июля 2025
        // amount2: -, //8 марта 2025
        amount3: '-', //ДР августа 2025
        amount4: '-', //ДР сентября 2025
        //   amount5: '-', //Пасха 2025
        //  amount6: 0, //9 мая 2025
      },
      {
...
async function quarta() {
  let orders = await Order.find({
    ...
    { "holiday": 'Дни рождения мая 2025' }, { "holiday": 'Дни рождения июля 2025' },
...
router.get("/report/:userName", checkAuth, async (req, res) => {
  try {
    ...
    let orderAmount = await Order.countDocuments({ userName: userName, isDisabled: false, dateOfOrder: { $gt: new Date("2025-06-30"), $lt: new Date("2025-08-01") } });
    let celebratorsAmount = await Order.aggregate(
      [
        {
          $match: { userName: userName, isDisabled: false, dateOfOrder: { $gt: new Date("2025-06-30"), $lt: new Date("2025-08-01") } }
        },
        {
          $group: { _id: null, sum_val: { $sum: "$amount" } }
        }
